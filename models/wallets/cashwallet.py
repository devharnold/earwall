...
class CashWallet:
    ...
    def deactivate_wallet(cls, user_email: str, wallet_id: int, expiration_days: int, datetime):
        try:
            ...
            cursor.execute("""
                UPDATE cash_wallet
                SET status = 'inactive', deactivated_at = %s, expiration_days = %s
                WHERE wallet_id = %s
                """,
                (deactivation_time, expiration_days, wallet_id)
            )
            connection.commit()
            ...
            return jsonify({"message": "Account successfully deactivated"}), 200
        except Exception as e:
            return jsonify({"error": str(e)}), 500
        finally:
            cursor.close()
